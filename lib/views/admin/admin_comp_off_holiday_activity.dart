...
import 'mass_approve_comp_off_activity.dart';

class AdminCompOffHolidayPage extends StatefulWidget {
  const AdminCompOffHolidayPage({super.key});

  @override
  State<AdminCompOffHolidayPage> createState() => _AdminCompOffHolidayPageState();
}

class _AdminCompOffHolidayPageState extends State<AdminCompOffHolidayPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: Constants.appBar(Constants.compOffHolidayRequest),
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Container(
                                                        alignment: Alignment.center,
                                                        width: 20.0,child: Text(Constants.colan, style: Constants.textStyle2(Constants.company),)),
                                                    if (int.parse(Filter().filterAdminCompOffHoliday(adminCompOffHolidayList, type)[index].requestType) == 1)
                                                      SizedBox(
                                                          width: textWidth2,
                                                          child: Text(
                                                            Constants.compOffC,
  ...
  Future<void> massApprove(BuildContext context) async {
    ...
    final result = await Navigator.push(
        context,
        // Create the SelectionScreen in the next step.
        MaterialPageRoute(builder: (context) => const MassApproveCompOffHolidayPage()
        )
    );
  ...
  void updateViewed() async {
    Map<String, String> data = {
      ...
      Constants.staffId     : staffId,
      Constants.type        : Constants.key101,
    };
    var client = Client();
    var response;
    try {
  ...
  void updateViewed2() async {
    Map<String, String> data = {
      Constants.requestType : Constants.updateViewed,
      Constants.staffId     : staffId,
      Constants.type        : Constants.key102,
    };
    var client = Client();
    var response;
    try {
